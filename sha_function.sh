...
compare (){
    ...
    if [ $result_arm == "false" ] || [ $result_arm64 == "false" ];     #compare alpine
    then
        echo "true"
    else
        echo "false"
...
create_manifest (){
    local repo=$1           #treehouses/sysmon
    local tag_latest=$2     #latest
    local tag_time=$3       #timetag
    local tag_arm=$4        #treehouses/sysmon-tags:arm
    local tag_arm64=$5

    docker manifest create   $repo:$tag_latest      $tag_arm $tag_arm64
    docker manifest create   $repo:$tag_time        $tag_arm $tag_arm64
